...
#def gerar_baralho(qtd_baralhos, coringas, embaralhar):
    ...
    #baralho = baralho * qtd_baralhos
    
    #return baralho
    
import random
    
def gerar_baralho(qtd_baralhos=2, coringas=True, embaralhado=True):
    baralho = []
    
    naipes = list('♠♥♦♣')
    numeros = list('A23456789') + ['10'] + list("QJK")
...
def mostrar_baralho(baralho):
    qtd_cartas = len(baralho)
    print(f'Há {qtd_cartas} cartas no baralho')
    print('Cartas:')
    print(', '.join(baralho))
    
def dar_as_cartas(baralho,num_jogadores=5,num_cartas=5):
    jogadores = {}
    
    for i in range(num_jogadores):
        mao = []
        while len(mao) <= num_cartas:
            carta = baralho.pop(0)  
            mao.append(carta)
